 ...
 * apibuilder:0.12.52 https://app.apibuilder.io/flow/catalog/0.3.65/play_2_x_standalone_json
 */
package io.flow.catalog.v0.models {
   ...
   * 
   * @param description Details of what the item function actually does
   * @param key Unique key to identify this function. If not specified, generated server side as
   *        the lowercase, hex encoded, Sha-256 of the source code itself
   */
   ...
   * @param position Functions are ordered. Lower position numbers indicate function that should be
   *        applied earlier.
   */
  ...
  /**
   * @param itemFunctionId The Id of the item function to associate with this subcatalog
   * @param q Optionally apply this pricing function only to the items that match this query.
   *        If not specified, the pricing function will apply to all items.
   * @param position Functions are ordered. Lower position numbers indicate function that should be
   *        applied earlier. Defaults to last in list.
   */
  ...
  object AttributeIntent {
     ...
     * above.
     */
    val all: scala.List[AttributeIntent] = scala.List(Brand, FulfillmentMethod, Hazardous, Price, Taxability)
